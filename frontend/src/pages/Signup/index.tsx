    ...
    >
      <div className="w-full max-w-sm">
        ...
        <div className="flex items-center gap-2.5 mb-10">
          <div className="flex items-center justify-center rounded" style={{ width: '28px', height: '28px', backgroundColor: '#FFFFFF' }}>
            <Building2 className="w-4 h-4" style={{ color: '#000000' }} />
          </div>
          <span className="text-sm font-semibold" style={{ color: '#FFFFFF' }}>CoimbatoreREI</span>
        </div>
        ...
        {formError && (
          <div className="flex items-start gap-2.5 p-3 rounded mb-6 text-sm" style={{ backgroundColor: '#0A0A0A', border: '1px solid #2A2A2A', color: '#A1A1AA' }}>
        ...
        <button
          ...
          className="flex items-center justify-center gap-2.5 w-full h-10 rounded text-sm font-medium transition-colors"
          style={{ backgroundColor: '#0A0A0A', border: '1px solid #2A2A2A', color: '#FFFFFF', opacity: isGoogleLoading ? 0.6 : 1 }}
          onMouseEnter={(e) => { if (!isLoading && !isGoogleLoading) (e.currentTarget as HTMLElement).style.backgroundColor = '#111111'; }}
          onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.backgroundColor = '#0A0A0A'; }}
        >
          {isGoogleLoading ? (
            <span className="inline-block rounded-full border-2" style={{ width: '14px', height: '14px', borderColor: '#FFFFFF', borderTopColor: 'transparent', animation: 'spin 0.7s linear infinite' }} />
          ) : <GoogleIcon />}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: '#A1A1AA' }}>Full Name</label>
            <input
              type="text"
          ...
          <button
            ...
            style={{ backgroundColor: '#FFFFFF', color: '#000000', opacity: isLoading ? 0.7 : 1 }}
            onMouseEnter={(e) => { if (!isLoading && !isGoogleLoading) (e.currentTarget as HTMLElement).style.backgroundColor = '#D4D4D4'; }}
            onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.backgroundColor = '#FFFFFF'; }}
          >
            {isLoading && (
              <span className="inline-block rounded-full border-2" style={{ width: '14px', height: '14px', borderColor: '#000000', borderTopColor: 'transparent', animation: 'spin 0.7s linear infinite' }} />
            )}
            Create Account
          </button>
        </form>
        ...
        <p className="text-xs text-center mt-6" style={{ color: '#71717A' }}>
          ...
          <Link to="/login" className="font-medium transition-colors" style={{ color: '#A1A1AA' }}
