    ...
    <div className="flex-1 flex flex-col md:flex-row" style={{ minHeight: 0 }}>
      ...
      >
        ...
        <div className="flex items-center gap-1.5 text-xs mb-6" style={{ color: '#52525B' }}>
          ...
          <ChevronRight className="w-3 h-3" />
          <span style={{ color: '#A1A1AA' }}>{currentLocality.name}</span>
        </div>

        {/* Title */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-1" style={{ color: '#FFFFFF', letterSpacing: '-0.03em' }}>
        ...
        {/* Score grid */}
        <div className="grid grid-cols-5 gap-0 mb-8" style={{ border: '1px solid #1F1F1F', borderRadius: '8px', overflow: 'hidden' }}>
          {scoreItems.map((s, idx) => (
            <div
              ...
              style={{
                ...
                borderRight: idx < scoreItems.length - 1 ? '1px solid #1F1F1F' : 'none',
              }}
            >
              <div className="text-xs uppercase tracking-wider mb-1" style={{ color: '#52525B' }}>{s.label}</div>
              <div className="text-xl font-bold" style={{ color: '#FFFFFF' }}>{s.value.toFixed(0)}</div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-3 gap-6 mb-8">
          {[
            ...
            <div key={m.label} className="flex flex-col gap-0.5">
              <span className="text-xs uppercase tracking-wider" style={{ color: '#52525B' }}>{m.label}</span>
              <span className="text-2xl font-semibold" style={{ color: '#FFFFFF' }}>{m.value}</span>
            </div>
          ))}
        </div>

        {/* Chart tabs */}
        <div style={{ borderBottom: '1px solid #1F1F1F', marginBottom: '24px' }}>
          <div className="flex gap-0">
            {([['1y', '1 Year'], ['3y', '3 Years'], ['dist', 'Distribution']] as [ChartTab, string][]).map(([key, label]) => (
              <button
                ...
                style={{
                  color: chartTab === key ? '#FFFFFF' : '#71717A',
                  borderBottom: chartTab === key ? '1px solid #FFFFFF' : '1px solid transparent',
                  marginBottom: '-1px',
                }}
              >
                {label}
          ...
          <ResponsiveContainer width="100%" height="100%">
            {chartTab === 'dist' ? (
              <BarChart data={priceDistribution} margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
                <XAxis dataKey="range" stroke="#3F3F46" fontSize={11} tickLine={false} axisLine={false} />
              ...
              <AreaChart data={areaChartData} margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
                ...
                <YAxis stroke="#3F3F46" fontSize={11} tickLine={false} axisLine={false} tickFormatter={(v) => `${v.toLocaleString()}`} width={60} />
                <Tooltip contentStyle={tooltipStyle} cursor={{ stroke: '#2A2A2A' }} />
                <Area type="monotone" dataKey="price" stroke="#FFFFFF" strokeWidth={1.5} fill="url(#areaGrad)" />
              </AreaChart>
            )}
          </ResponsiveContainer>
        </div>
        ...
        {/* Amenities */}
        <div style={{ borderTop: '1px solid #1F1F1F', paddingTop: '32px' }}>
          <h2 className="text-base font-semibold mb-4" style={{ color: '#FFFFFF' }}>Nearby Amenities</h2>
          <div className="flex gap-0 mb-6" style={{ borderBottom: '1px solid #1F1F1F' }}>
            {amenityCategories.map(({ key, label, icon: Icon }) => (
              <button
                ...
                style={{
                  color: amenityTab === key ? '#FFFFFF' : '#71717A',
                  borderBottom: amenityTab === key ? '1px solid #FFFFFF' : '1px solid transparent',
                  marginBottom: '-1px',
                }}
              >
          ...
          <div className="space-y-0">
            {getAmenities(amenityTab).map((a, idx) => (
              ...
              >
                <span className="text-sm" style={{ color: '#FFFFFF' }}>{a.name}</span>
        ...
        {/* Recommendations */}
        <div style={{ marginTop: '40px', borderTop: '1px solid #1F1F1F', paddingTop: '32px' }}>
          <h2 className="text-base font-semibold mb-4" style={{ color: '#FFFFFF' }}>Similar Localities</h2>
          <div className="flex gap-0 mb-6" style={{ borderBottom: '1px solid #1F1F1F' }}>
            {([['similar', 'Similar'], ['CHEAPER', 'Budget'], ['PREMIUM', 'Premium'], ['HIGH_GROWTH', 'Growth']] as [RecTab, string][]).map(([key, label]) => (
              <button
                ...
                className="px-4 py-2.5 text-sm font-medium transition-colors"
                style={{
                  color: recTab === key ? '#FFFFFF' : '#71717A',
                  borderBottom: recTab === key ? '1px solid #FFFFFF' : '1px solid transparent',
                  marginBottom: '-1px',
                }}
              >
                {label}
          ...
          <div className="space-y-0">
            {getRecs(recTab).map((rec: any) => (
              <div
                ...
                style={{ borderBottom: '1px solid #111111' }}
                onClick={() => {/* navigate to locality */}}
                onMouseEnter={(e) => ((e.currentTarget as HTMLElement).style.backgroundColor = '#0A0A0A')}
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span className="text-xs font-semibold flex-shrink-0" style={{ color: '#A1A1AA' }}>
      ...
      <div
        ...
        style={{ width: '420px', flexShrink: 0, borderLeft: '1px solid #1F1F1F' }}
      >
